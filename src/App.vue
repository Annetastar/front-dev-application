      ...
      <div class="main-page">
        <div class="main-page__left">
          ...
          <div class="menu">
              ...
              <div>
                <a href="javascript();">
                  <img src="./images/translations.svg" alt="translations">
                  <h3>Трансляции</h3>
                </a>
              </div>
              <div>
            ...
            <div class="filters">
                ...
                <div class="calendar-filter">
                  <ul>
                    <li :class="{active_btn: filter === 'all'}">
                      <a href="#" @click="(filter = 'all', removeDisabled())">Весь срок</a>
                    </li>
                    <li :class="{active_btn: filter === 'today'}">
                      <a href="#" @click="(filter = 'today', removeDisabled())">Сегодня</a>
                    </li>
                    <li :class="{active_btn: filter === 'yesterday'}">
                      <a href="#" @click="(filter = 'yesterday', removeDisabled())">Вчера</a>
                    </li>
                    <li :class="{active_btn: filter === 'sevenDays'}">
                      <a href="#" @click="(filter = 'sevenDays', removeDisabled())">Последние 7 дней</a>
                    </li>
                    <li :class="{active_btn: filter === 'thirtyDays'}">
                      <a href="#" @click="(filter = 'thirtyDays', removeDisabled())">Последние 30 дней</a>
                    </li>
                    <li :class="{active_btn: filter === 'thisMonth'}">
                      <a href="#" @click="(filter = 'thisMonth', removeDisabled())">В этом месяце</a>
                    </li>
                    <li :class="{active_btn: filter === 'exMonth'}">
                      <a href="#" @click="(filter = 'exMonth', removeDisabled())">Прошлый месяц</a>
                    </li>
                  </ul>
                </div>
              <div class="table-calendar">
                  <div class="top-row">
                    ...
                    <ul class="days">
                      <li v-for="(d, indexDay) in day" :key='indexDay'>{{d}}</li>
                    </ul>
                  </div>
                <table>
                  <tr v-for="(week, i) in calendar()" :key='i'>             
                    <td v-for="(day, index) in week" :key='index'
                    :class="{active: day.index === new Date().getDate() && new Date().getMonth() === month}"> {{ day.index }} </td>
                  </tr>            
                </table>
                <div class="table-calendar_bottom">
                  <a href="#" class="reset" @click="reset">Отмена</a>
                  <a href="#" class="reload disabled" @click="reload">Обновить</a>
                </div>
              </div>
              <div class="blur"></div>
              </div>
              ...
              <ul>
                <li v-for="(item, index) in filteredUsers" :key="index">
                  <ul>
                    ...
                    <li>
                      <span>{{monthes[item.registration.split('.')[1] - 1]  + ' ' + item.registration.split('.')[0] + ', ' + '20' + item.registration.split('.')[2]}}</span>
                    </li>
                    <li>
                      <span>{{monthes[item.active.split('.')[1] - 1]  + ' ' + item.active.split('.')[0] + ', ' + '20' + item.active.split('.')[2]}}</span>
                    </li>
                    <li>
                      <span>{{item.act}}</span>
                    </li>
                    ...
                    <div class="icons">
                      ...
                      <a href="#">
                        <img src="./images/backet.svg" alt="backet">
                      </a>
                    </div>
                  </ul>
                </li>
...
computed: {
  filteredUsers() {
    ...
    if (this.filter === 'all') {
      ...
      return this.dataFromServer.filter(el => Number(el.active.split('.')[0]) === nowDateVariable && Number(el.active.split('.')[1]-1) === nowMonthVariable);
    } else if (this.filter === 'yesterday') {
      return this.dataFromServer.filter(el => Number(el.active.split('.')[0]) === nowDateVariable - 1 && Number(el.active.split('.')[1]-1) === nowMonthVariable);
    } else if (this.filter === 'sevenDays') {
      return this.dataFromServer.filter(el => Number(el.active.split('.')[0]) >= nowDateVariable - 7 && Number(el.active.split('.')[1]-1) === nowMonthVariable);
    } else if (this.filter === 'thirtyDays') {
      return this.dataFromServer.filter(el => Number(el.active.split('.')[0]) >= nowDateVariable - 30);
    } else if (this.filter === 'thisMonth') {
      return this.dataFromServer.filter(el => Number(el.active.split('.')[1]-1) === nowMonthVariable);
    } else if (this.filter === 'exMonth') {
      return this.dataFromServer.filter(el => Number(el.active.split('.')[1]-1) === nowMonthVariable - 1);
    }
    return this.dataFromServer;
  }
}
